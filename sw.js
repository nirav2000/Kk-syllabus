const C='curious-path-v7-learning-evidence';
const A=['./','./index.html','./styles.css','./manifest.webmanifest','./garden.css',
  ...['app','engine','content','maths-data','words-data','word-engine','word-lab','attempts','quiz','reports','explanations','garden','garden-model','garden-picture','local-data','data-store','profiles','profile-panel','sync-model','firebase-config','cloud-sync','sync-transport'].map(name=>`./src/${name}.js`)];
