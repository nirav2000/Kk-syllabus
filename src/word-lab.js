...
import { mountVoiceSpelling } from './voice-spelling.js';
...
export function openWords(root,onExit,mode='spelling') {
  ...
  const allowVoice=!!p.events.filter(e=>e.type==='word-settings').at(-1)?.data.allowVoice;
  let index=0,clock,help=false,disposed=false,utterance,voice,canAnswer=mode!=='spelling',parentPaused=false,inputMethod='typed';
  const pause=()=>{parentPaused=true;clock?.pause();speechStop();},resume=()=>{parentPaused=false;if(!document.hidden)clock?.resume();};
  function speechStop(){if('speechSynthesis' in window)window.speechSynthesis.cancel();voice?.stop();}
  ...
  function dispose(){disposed=true;voice?.dispose();document.removeEventListener('visibilitychange',visibility);clock?.pause();speechStop();}
  ...
  function draw(){voice?.dispose();voice=null;if(!items[index])return close();const word=items[index];help=false;inputMethod='typed';canAnswer=mode!=='spelling';clock=createClock();if(document.hidden)clock.pause();
    ...
    if(mode==='spelling'){
      if(allowVoice){const section=document.createElement('section');section.className='voice-spelling';root.querySelector('form').after(section);voice=mountVoiceSpelling(section,{onConfirm:letters=>{root.querySelector('input').value=letters;inputMethod='voice-confirmed';}});}
      ...
      root.querySelector('input').oninput=()=>{inputMethod='typed';};
    ...
    function submit(response){if(disposed||submitted)return;submitted=true;speechStop();const timing=clock.read();clock.pause();const expected=mode==='spelling'?word.word:word.meaning,correct=mode==='spelling'?normalizeSpelling(response)===word.word:response===expected;
      ...
      voice?.dispose();
      const event=answerEvent({...timing,wordId:word.id,itemId:`${word.id}-${mode}`,topic:mode==='spelling'?'Spelling':'Vocabulary',mode,inputMethod:mode==='meaning'?'choice':inputMethod,stem:mode==='spelling'?`Spell ${word.word}`:`What does ${word.word} mean? ${word.sentence}`,response:response.slice(0,200),expected,correct,supported:help,choices,sessionId,position:index,feedback},'word-answer');
...
export function mountWordReport(root,profile,onSave) {
  ...
  const voiceLabel=document.createElement('label');voiceLabel.className='voice-opt-in';voiceLabel.innerHTML='<input type="checkbox" name="voice"> Allow experimental microphone letter spelling. The browser’s recognition provider may process audio online. This app stores only the confirmed spelling, not audio or the raw transcript. Check recognition on your iPhone/iPad before relying on it.';voiceLabel.querySelector('input').checked=!!settings.allowVoice;root.querySelector('form button').before(voiceLabel);
  root.querySelector('[data-mode]').onchange=render;root.querySelector('[data-state]').onchange=render;
  root.querySelector('form').onsubmit=e=>{e.preventDefault();const minYear=+e.target.elements.min.value,maxYear=+e.target.elements.max.value;if(minYear>maxYear){root.querySelector('[data-range-status]').textContent='The lowest band must not exceed the highest.';return;}onSave(answerEvent({minYear,maxYear,allowVoice:e.target.elements.voice.checked},'word-settings'));};render();
}
