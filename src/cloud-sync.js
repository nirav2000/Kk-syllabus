...
export function mountCloudPanel(element) {
  element.innerHTML='<h3>Save across devices</h3><p class="small">One shared learner profile for your family. Sign in with the same parent account on each trusted device to combine its local progress with the cloud copy.</p><p class="small" data-status role="status"></p><form class="cloud-form"><label>Parent email<input name="email" type="email" autocomplete="username" required></label><label>Password<input name="password" type="password" autocomplete="current-password" required></label><button class="primary">Sign in & sync</button></form><div data-connected><button data-sync>Sync now</button> <button data-disconnect>Disconnect cloud</button><p class="small">Disconnecting keeps local progress. Reset this device before handing it to someone else.</p></div>';
...
if(typeof window!=='undefined'){
  window.addEventListener('curious-data',e=>{if(e.detail.source==='local'&&state.signedIn)schedule();});
