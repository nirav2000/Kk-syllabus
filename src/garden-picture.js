...
export function gardenPicture(width, height, mode = 'area', edge = 3) {
  ...
  const highlight = mode === 'edge' ? sides.slice(0, edge + 1).map(([x1,y1,x2,y2]) => `<line data-boundary="true" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="#874808" stroke-width="5" stroke-linecap="round"/>`).join('') : '';
