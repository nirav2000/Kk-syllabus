...
| Spelling | Tap-to-hear word and sentence; typed answer; reveal-to-practise; calm correction | Audio depends on device/browser voice availability |
| Voice spelling (experimental) | Parent opt-in; tap to say letter names; stop microphone; confirm decoded letters before checking; typing fallback | Uses browser recognition, which may process audio remotely. No audio/raw transcript stored. Actual iPhone/iPad recognition accuracy is not yet verified |
...
| Profiles | Persistent nickname/type bar; PIN-protected switch shortcut; explicit current-profile card; separate new-profile form defaulting to Learner; safe test resets | PIN is local privacy protection, not a strong security boundary. Changing the new-profile type never edits an existing profile |
...
4. **Voice spelling validation on iPhone/iPad:** the opt-in experimental interface is implemented, with explicit start/stop, letter-name transcript confirmation and keyboard fallback. Test actual Safari microphones, B/D/P/T and M/N confusion, recognition-provider privacy behaviour and interruption handling before relying on it. Do not treat whole-word dictation as spelling evidence; the parser rejects it.
